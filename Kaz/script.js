...
// Base API URL
const API_BASE = 'http://18.130.172.110/api/';
//const API_BASE = 'http://18.175.173.16/api/';
//const API_BASE = 'api\';
...
async function completeOrder(orderId, completedBy) {
    try {
        const response = await fetch(API_BASE + 'complete_order.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ orderId, completedBy }),
        });
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    if (result.success) {
        ...
        document.getElementById('navbarUsername').textContent = result.user.username;
...
// Update the dashboard to fetch from database
async function updateDashboard() {
    const orders = await getOrders();
    populateOrdersTable(orders);
    updateStats(orders);
...
function viewOrder(orderId) {
    ...
    if (order) {
        ...
        // Create a modal for order details with PDF export option
        const modalHTML = `
            <div class="modal fade" id="orderModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Order Details - ${order.clientName}</h5>
                            ...
                            ${order.floors.map(floor => `
                                ...
                                ${floor.rooms.map(room => `
                                    <div style="margin-left: 20px;">
                                        <h7>${room.name}</h7>
                                        <ul>
                                            ${room.items.map(item => `<li>${item.service} (Quantity: ${item.quantity})</li>`).join('')}
                                        </ul>
                                    </div>
                                `).join('')}
                            `).join('')}
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary" onclick="exportToPDF(${JSON.stringify(order).replace(/"/g, '&quot;')})">Export as PDF</button>
                        </div>
                    </div>
                </div>
            </div>
...
async function completeOrder(orderId) {
    ...
    if (confirm('Are you sure you want to mark this order as completed?')) {
        const result = await completeOrder(orderId, user.username);
...
function exportToPDF(order) {
    ...
    printableElement.innerHTML = `
        <div style="text-align: center; margin-bottom: 20px;">
            <h1 style="color: #000; margin-bottom: 10px; font-size: 28px;">Kazkleen</h1>
            <h2 style="color: #333; font-size: 20px;">Cleaning Project Overview</h2>
            <hr style="border-top: 2px solid #000;">
        </div>
        <div style="margin-bottom: 20px;">
            <h3 style="font-size: 18px;">Client: ${order.clientName}</h3>
            <p><strong>Date:</strong> ${order.date}</p>
            <p><strong>Submitted by:</strong> ${order.submittedBy}</p>
        </div>
    `;
    ...
    order.floors.forEach(floor => {
        printableElement.innerHTML += `
            <div style="margin-bottom: 15px;">
                <h4 style="font-size: 16px; background-color: #f0f0f0; padding: 5px; border-radius: 5px;">${floor.name}</h4>
        `;
        
        floor.rooms.forEach(room => {
            printableElement.innerHTML += `
                <div style="margin-left: 20px; margin-bottom: 10px;">
                    <h5 style="font-size: 14px;">${room.name}</h5>
                    <ul style="list-style-type: none; padding-left: 0;">
            `;
            
            room.items.forEach(item => {
                printableElement.innerHTML += `<li style="margin-bottom: 5px;">• ${item.service} (Quantity: ${item.quantity})</li>`;
            });
    ...
    if (order.status === 'completed') {
        printableElement.innerHTML += `
            <div style="margin-top: 30px; padding-top: 10px; border-top: 1px solid #ccc;">
                <h3 style="font-size: 16px;">Project Completion</h3>
                <p><strong>Completed on:</strong> ${order.completedDate}</p>
                <p><strong>Completed by:</strong> ${order.completedBy || 'Manager'}</p>
            </div>
        `;
    ...
    printableElement.innerHTML += `
        <div style="text-align: center; margin-top: 30px;">
            <button onclick="window.print()" style="background-color: #000; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">Print / Save as PDF</button>
        </div>
    `;
    ...
    printWindow.document.write(`
        <html>
            <head>
                <title>Kazkleen Order - ${order.clientName}</title>
                <style>
